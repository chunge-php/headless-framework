...
class SmsChannel
{
    ...
    {
        colorText('短信分组生成' . dayDateTime(), 'green');
        // 每 60 秒巡逻一次
    ...
    public function getGroupUsersId()
    {
        ...
        $id =  Db::table('group_users')->where('uid', config('app.sms_quick_uid'))->where('channel_type', state_one)->orderBy('id', 'desc')->first();
        if (!$id) {
            ...
            if ($id->total >= 10000) {
                $sort = Db::table('group_users')->where('uid', config('app.sms_quick_uid'))->where('channel_type', state_one)->count();
                $sort++;
                $id =  Db::table('group_users')->insertGetId([
